...
function updateGalacticPricesSheet() {
  var pricesData = getGalacticPrices();
  ...
  var sheet = spreadsheet.getSheetByName('galactic-prices');

  // If the 'galactic-prices' sheet does not exist, create it
  if (!sheet) {
    sheet = spreadsheet.insertSheet('galactic-prices');
  }
